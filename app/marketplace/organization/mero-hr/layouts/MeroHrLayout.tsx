...
import MeroHrSidebar from '../components/MeroHrSidebar';

export default function MeroHrLayout() {
    const { theme } = useTheme();
    const { organization } = useAuthStore();
    const location = useLocation();

    const [sidebarCollapsed, setSidebarCollapsed] = useState(() => {
        const saved = localStorage.getItem('mero-hr-sidebar-collapsed');
        return saved === 'true';
    });

    // Listen for sidebar toggle events from header if needed (matching other apps)
    useEffect(() => {
        const handleSidebarToggle = (e: CustomEvent) => {
            setSidebarCollapsed(e.detail.collapsed);
        };
        window.addEventListener('mero-hr-sidebar-toggle', handleSidebarToggle as EventListener);
        return () => window.removeEventListener('mero-hr-sidebar-toggle', handleSidebarToggle as EventListener);
    }, []);

    const toggleCollapsed = () => {
        const newState = !sidebarCollapsed;
        setSidebarCollapsed(newState);
        localStorage.setItem('mero-hr-sidebar-collapsed', String(newState));
        window.dispatchEvent(new CustomEvent('mero-hr-sidebar-toggle', { detail: { collapsed: newState } }));
    };
    ...
    const checkActive = (href: string, currentPath: string): boolean => {
        ...
        if (isSubdomainRoute) {
            ...
            const appBase = `/app/mero-hr`;
            const orgAppBase = `/org/${organization?.slug}/app/mero-hr`;
    ...
    const buildHref = (href: string): string => {
        if (isSubdomainRoute) {
            ...
            const base = `/org/${organization?.slug}/app/mero-hr`;
