...
export default function IncomePage() {
    ...
    const { data: entries = [], isLoading } = useQuery({
        queryKey: ['khata-entries', 'INCOME', startDate, endDate],
        queryFn: () => khataService.getEntries('INCOME', startDate || undefined, endDate || undefined),
    });
    ...
    const createMutation = useMutation({
        mutationFn: (data: any) => khataService.createEntry({ ...data, type: 'INCOME' }),
        onSuccess: () => {
        ...
        <div className="p-6 max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold" style={{ color: theme.colors.text }}>Income</h1>
                        <p className="text-sm" style={{ color: theme.colors.textSecondary }}>Track all your income entries</p>
                    </div>
                </div>
                <button
            ...
            {/* Summary Card */}
            <div className="rounded-xl p-4 mb-6 bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800">
            ...
            {showForm && (
                <div className="rounded-xl border p-6 mb-6" style={{ borderColor: theme.colors.border, backgroundColor: theme.colors.surface }}>
                    <h3 className="text-lg font-semibold mb-4" style={{ color: theme.colors.text }}>Add Income Entry</h3>
            ...
            {isLoading ? (
                ...
                <div className="text-center py-16 rounded-xl border" style={{ borderColor: theme.colors.border }}>
                    <TrendingUp className="w-12 h-12 mx-auto mb-3 opacity-30" style={{ color: theme.colors.textSecondary }} />
                    <p className="font-medium" style={{ color: theme.colors.text }}>No income entries yet</p>
                    <p className="text-sm mt-1" style={{ color: theme.colors.textSecondary }}>Click "Add Income" to record your first entry</p>
                </div>
            ) : (
